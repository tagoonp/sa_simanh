...
$ageRange = array(' < 18 ',' 18-19 ',' 20-34 ',' 35 yr and older');
...
if($query==0){	//Daily
	...
	if($view==0){	//All hospital
		...
		$max = 1;
		$ageRangeSQL = array(' < 18',' between 18 and 19 ',' between 20 and 34 ',' > 35 ');
		//print sizeof($hosx);
		foreach($ageRangeSQL as $h){
			$hosEachMon = array();
			foreach($dateRa as $eachDate){
				if($case==1){
					$strSQL = "SELECT count(*) FROM fmn1_registerrecord WHERE username in (SELECT username FROM fmn1_userdescription WHERE institute_id in $hoslist) and p_cage $h and date_adm like '".$eachDate."-%' and confirm_status = 1";
				}else if($case==2){
					$strSQL = "SELECT count(*) FROM fmn1_registerrecord a inner join fmn1_delivery b on a.record_id = b.record_id
							   WHERE a.username in (SELECT username FROM fmn1_userdescription WHERE   institute_id in $hoslist) and a.p_cage $h and a.date_adm like '".$eachDate."-%' and a.confirm_status = 1";
				}else if($case==3){
					$strSQL = "SELECT count(*) FROM fmn1_registerrecord a inner join fmn1_delivery b on a.record_id = b.record_id
								inner join fmn1_outcome c on a.record_id = c.record_id 
							   WHERE a.username in (SELECT username FROM fmn1_userdescription WHERE   institute_id in $hoslist) and a.p_cage $h and a.date_adm like '".$eachDate."-%' and a.confirm_status = 1";
					...
					$strSQL = "SELECT count(*) FROM fmn1_registerrecord a inner join fmn1_delivery b on a.record_id = b.record_id
								inner join fmn1_outcome c on a.record_id = c.record_id 
							   WHERE a.username in (SELECT username FROM fmn1_userdescription WHERE   institute_id in $hoslist) and a.p_cage $h and a.date_adm like '".$eachDate."-%' and a.confirm_status = 1 and c.alive = 1";
				}else if(($case>4) and ($case<=20)){
					//$com = $case-4;
					switch($case){
						case 5: $c = 'il';break;
					...
					$strSQL = "SELECT count(*) FROM fmn1_registerrecord a inner join fmn1_complication_delivery b 
							   on a.record_id = b.record_id
							   WHERE a.username in (SELECT username FROM fmn1_userdescription WHERE   institute_id in $hoslist) 
							    and a.p_cage $h and a.date_adm like '".$eachDate."-%' and a.confirm_status = 1 
							   and b.$c = 1";
					//print $strSQL."<br>";
				}else if(($case>=21) and ($case<=24)){
					switch($case){
