...
//print gettype($_POST['refer_to']);
//print $_POST['nrefer_to'][0];
//exit();
//If privilegde available
if($resultUser){
	$strSQL = sprintf("DELETE FROM ".substr(strtolower($tbf),0,-2)."%s  
				WHERE record_id = '%s'",
	...
	$strSQL = sprintf("INSERT INTO ".substr(strtolower($tbf),0,-2)."%s 
				...
				mysql_real_escape_string($_POST['refer_date']),
	...
	if($resultInsert){
		...
		if($resultCheck){
			...
			$resultDelete2 = $db->delete($strSQL);
			...
			?>
			<script>
					alert('Update postnatal information complete!');
