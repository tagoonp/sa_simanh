...
if($resultUser){
	$strSQL = "SELECT * FROM ".substr(strtolower($tbf),0,-2)."registerrecord
				WHERE (pid like '".$key."%' or p_fname like '%".$key."%'  or p_lname like '%".$key."%'  or point_no = '%".$key."%'
				 or folder_no = '%".$key."%') and username in (SELECT username FROM ".substr(strtolower($tbf),0,-2)."userdescription WHERE institute_id in (SELECT institute_id FROM ".substr(strtolower($tbf),0,-2)."userdescription WHERE username = '".$_SESSION['userSIMANHusername']."'))";
	...
	if($resultSearch){
		...
		foreach($resultSearch as $v){
            ...
            <strong><a href="summary.php?id=<?php print $v['record_id'];?>" class="d">View</a></strong>           </tr>
			<tr>
